...
public class LayoutImplIE8 extends LayoutImpl {
	...
	public void layout(Layer layer) {
		...
		// layout() is run again for it. This is because the translation of
		// units
		// to pixel values will be incorrect for invisible elements, and thus
		// must
	...
	private void updateVisibility(Element container) {
		...
		// Walk all children, looking for elements with a '__layer' property. If
		// one
		// exists, call layout() for that element. This is not cheap, but it's
		// the
		// only way to correctly ensure that layout units get translated
		// correctly.
