...
/**
 * Panel containing all of the exercise administration tools
 * 
 * This code is a refactoring of the {@link ProblemAdminPage} to fit into a tab.
 * Most of the work has involved passing a reference to the containing {@link CloudCoderPage},
 * in this case {@link CoursesAndProblemsPage3}, to the constructor.
 * 
 * mechanism used by its containing page?
 * 
 * @author jspacco
 *
 */
public class ExerciseAdminPanel extends Composite implements SessionObserver, Subscriber {
    ...
    public ExerciseAdminPanel(CloudCoderPage p) {
        this.page=p;
    ...
    /**
     * Convenience method that delegates
     * @param r
     */
    private void recoverFromServerSessionTimeout(Runnable r){
        this.page.recoverFromServerSessionTimeout(r);
    }
    ...
    private Course getCurrentCourse() {
        ...
        if (courseSelection==null) {
            GWT.log("Course selection is null!");
        }
    ...
    private void doChangeVisibility(final boolean visible) {
        ...
        for (Problem problem : chosen) {
            SessionUtil.loadProblemAndTestCaseList(this.page, problem, new ICallback<ProblemAndTestCaseList>() {
                /* (non-Javadoc)
    ...
    /**
     * Delegate to the encapsulated {@link CloudCoderPage}.
     * 
     * @param obj  object to add to the Session
     */
    private void addSessionObject(Object obj) {
        this.page.addSessionObject(obj);
    }
    ...
    private void handleEditProblem() {
        ...
        SessionUtil.loadProblemAndTestCaseList(this.page, chosen, new ICallback<ProblemAndTestCaseList>() {
            @Override
            public void call(ProblemAndTestCaseList value) {
                getSession().add(value);
    ...
    public void reloadProblems(final Course course) {
        // Reload problems
        SessionUtil.loadProblemAndSubmissionReceiptsInCourse(this.page, course, getSession());
    ...
    public void redisplay() {
        ...
        GWT.log("redisplay exercises in Admin panel");
        if (session==null) {
            GWT.log("session is null in exercise redisplay");
            return;
        }
        Course course=getCurrentCourse();
        if (course==null){
            GWT.log("course is null in exercise redisplay");
            return;
        }
        courseAdminProblemListView.loadProblems(session, course);
    }
}
